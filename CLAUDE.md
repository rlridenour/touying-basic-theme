...
- **Section slides center both ways; subsection slides don't.** This mirrors the original Beamer theme, where the section page used `\begin{centering}` but the subsection page had that line commented out. `new-section-slide` centers both ways, `new-subsection-slide` only vertically; each is built directly on `touying-slide`/`touying-slide-wrapper` (rather than delegating to the generic `centered-slide`/`left-slide` helpers) so it has direct `self` access for `self.store.title-color`. Don't merge the two, and don't route them back through `centered-slide`/`left-slide` — nesting a second slide-wrapper inside one of those would produce an inner, never-invoked wrapper instead of a real slide.
- **Colors are variant-driven, not hardcoded.** `basic-theme(variant: "white" | "black" | "gray" | "obu")` looks up `(bg, fg, accent)` from the `variant-colors` dict and threads them through `config-page(fill: bg)`, `config-colors(neutral-lightest: bg, neutral-darkest: fg, primary: fg)`, and `config-store(title-color: accent)`. Everywhere else in the file that needs body/heading text color reads `self.colors.neutral-darkest` (or relies on the ambient `set text(fill: ...)` from `config-methods(init: ...)`) rather than hardcoding black/white; the title-slide title, section/subsection headings, and `subslide-preamble` frame title instead read `self.store.title-color` (or the local `accent` variable, inside `basic-theme` itself) so only those elements pick up a variant's accent color. `accent` equals `fg` for every variant except `"obu"`, which keeps black-on-white body text but colors just those headings — so all variants stay in sync automatically. If you add new themed elements, follow the same pattern — don't hardcode a color.
