...
- **`rlr-touying-scaffold.el`** and **`ox-touying.el`** — Emacs tooling, tracked in git. `rlr-touying-scaffold.el` scaffolds a new presentation directory (`M-x rlr/new-touying-presentation`): `config.typ`, `talk.org`, the slides/handout entry points, and a `content.typ` generated from `talk.org`. `ox-touying.el` is the Org export backend that generates `content.typ` from an Org file (`M-x rlr/org-export-to-touying-content`) — heading depth maps 1:1 onto Typst heading depth, `#+begin_speakernote`/`#+begin_handoutnote` special blocks map to `#speaker-note[...]`/`#handout-note[...]`, `@@typst:...@@` export snippets (e.g. `@@typst:#pause@@`) pass through raw, and `#+begin_columns`/`#+begin_fullslide` special blocks cover the two-column and full-bleed layouts. Since talk.org is the authoring source, `content.typ` should be re-exported rather than hand-edited once ox-touying.el is in use.
